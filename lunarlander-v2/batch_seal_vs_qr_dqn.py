...
def compare_within_ckpt(kf, bc, config, working_directory,
    ...
    for ckpt in [i * int(1e4) for i in range(int(config['max_training_steps'] / 1e4))]:
        ...
        for idx in range(kf.n_splits):
            ...
            agent_idx = QuantileAgent(name=name, num_actions=num_actions, config=config_idx)
        ...
        agent = QuantileAgent(name=name, num_actions=num_actions, config=config)
        agent.model.load_weights(config['checkpoint_path']+'offline_qr_dqn_{}.ckpt'.format(ckpt))
