...
from peal.models.box2d_models import DQNNetwork
...
class DQNAgent:
    def __init__(self, name='LunarLander-v2',
        ...
        self.config = config
        # model & target model
        self.model = self.network(num_actions, config['hiddens'], config['activation'], config['dueling'])
        self.target_model = self.network(num_actions, config['hiddens'], config['activation'], config['dueling'])
        # optimizer
        ...
        for step_id in range(config['max_training_steps']):
            ...
            if self.training_steps % config['training_steps_to_checkpoint'] == 0:
                if config['double'] == False:
                    path = config['checkpoint_path'] + 'offline_dqn_{}.ckpt'.format(self.training_steps)
                else:
                    path = config['checkpoint_path'] + 'offline_ddqn_{}.ckpt'.format(self.training_steps)
                self.save(path)
                print('saving model weights at {}'.format(path))
    ...
    def _train(self):
        ...
        with tf.GradientTape() as tape:
            ...
            if config['double']:
                ...
                # Q_t(s', a_max)
                next_qt_vals = self.target_model(next_states).q_values
                next_vals = tf.gather_nd(next_qt_vals, indices=indices)
            else:
                # Q_t(s', argmax Q_t(s', a'))
                next_vals = tf.math.reduce_max(self.target_model(next_states).q_values, axis=1)
            
            is_non_terminal = 1. - tf.cast(dones, tf.float32)
            rewards = tf.clip_by_value(tf.cast(rewards, tf.float32), 
                                       -config['reward_clip'], 
                                        config['reward_clip'])
            targets = tf.stop_gradient(rewards + config['gamma'] * next_vals * is_non_terminal)
