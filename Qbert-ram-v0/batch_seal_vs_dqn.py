...
def compare_within_ckpt(kf, bc, config, working_directory,
    ...
    for ckpt in [i * int(1e4) for i in range(1, int(config['max_training_steps'] / 1e4) + 1)]:
        ...
        for idx in range(kf.n_splits):
            ...
            agent_idx.model.load_weights(kf.ckpt_paths[idx] + 'offline_dqn_{}.ckpt'.format(ckpt))
        ...
        agent.model.load_weights(config['checkpoint_path']+'offline_dqn_{}.ckpt'.format(ckpt))
...
def one_round_run(replica,
    ...
    #     file_path = 'dqn_{}_cv{}_rc{}_dt{}.jpg'.format(strategy, idx, replica, datetime.now().strftime('%Y%m%d_%H-%M-%s'))
    #     plt.savefig(os.path.join(pic_dir, file_path))
