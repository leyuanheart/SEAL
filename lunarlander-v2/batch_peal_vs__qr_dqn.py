...
def set_config(kf, lr=5e-4, decay_step=int(10e4), max_train_step=int(20e4), hidden_size=[256, 256]):
    config['online'] = False
    config['hiddens'] = hidden_size
    config['lr'] = lr
...
def compare_within_ckpt(kf, bc, config, working_directory,
    ...
    for ckpt in [i * int(1e4) for i in range(int(config['max_training_steps'] / 1e4))]:
        ...
        for idx in range(kf.n_splits):
            ...
            agent_idx.load(kf.ckpt_paths[idx] + '{}_{}.ckpt'.format(agent_name, ckpt))
        ...
        agent.load(config['checkpoint_path']+'{}_{}.ckpt'.format(agent_name, ckpt))
    ...
    ckpt_result_pdf = ckpt_result_pdf[['peal_mean_reward', 'dml_mean_reward', 'single_agent_mean_reward'] + ['cv{}'.format(i) for i in range(num_kf)]]
...
def one_round_run(replica,
    ...
    path = 'data/{}_{}_{}/{}/trajs_qr_dqn.pkl'.format(
        agent_name, n_splits, n_trajs, strategy, agent_name
    )
    ...
    file_path = '{}_{}_single_rc{}_dt{}.jpg'.format(agent_name, strategy, replica, datetime.now().strftime('%Y%m%d_%H-%M-%s'))
    ...
    for idx in range(kf.n_splits):
        ...
        file_path = '{}_{}_cv_rc{}_dt{}.jpg'.format(agent_name, strategy, replica, datetime.now().strftime('%Y%m%d_%H-%M-%s'))
        plt.savefig(os.path.join(pic_dir, file_path))
