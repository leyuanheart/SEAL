...
def set_config(kf, lr=5e-4, decay_step=int(1e4), max_train_step=int(5e4)):
    ...
    config['num_convex_combinations'] = 1
    # config['epsilon_end'] = 0.01
    # config['epsilon_decay_period'] = 1e4
...
def compare_within_ckpt(kf, bc, config, working_directory,
    ...
    for ckpt in [i * int(1e4) for i in range(1, int(config['max_training_steps'] / 1e4) + 1)]:
        ...
        for idx in range(kf.n_splits):
            ...
            agent_idx = MultiHeadDQNAgent(name=name, num_actions=num_actions, config=config_idx)
            agent_idx.model.load_weights(kf.ckpt_paths[idx] + 'offline_rem_{}.ckpt'.format(ckpt))
        ...
        agent = MultiHeadDQNAgent(name=name, num_actions=num_actions, config=config)
        agent.model.load_weights(config['checkpoint_path']+'offline_rem_{}.ckpt'.format(ckpt))
...
def one_round_run(replica,
    ...
    # file_path = 'rem_{}_single_rc{}_dt{}.jpg'.format(strategy, replica, datetime.now().strftime('%Y%m%d_%H-%M-%s'))
    ...
    #     file_path = 'rem_{}_cv{}_rc{}_dt{}.jpg'.format(strategy, idx, replica, datetime.now().strftime('%Y%m%d_%H-%M-%s'))
    #     plt.savefig(os.path.join(pic_dir, file_path))
...
# import pickle
# with open('trajs_qr_dqn.pkl', 'rb') as f:
#     trajs = pickle.load(f)